...
node --loader ts-node/esm scripts/checks/check-services.ts
if [ $? -ne 0 ]; then
...
node --loader ts-node/esm scripts/ops/purge-consumers.ts || echo "purge-consumers failed (non-fatal, continuing)."
