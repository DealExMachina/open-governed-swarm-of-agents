...
CHECK_FEED=1 CHECK_SERVICES_MAX_WAIT_SEC=300 node --loader ts-node/esm scripts/checks/check-services.ts
if [ $? -ne 0 ]; then
...
node --loader ts-node/esm scripts/ops/seed-all.ts
node --loader ts-node/esm scripts/ops/bootstrap-once.ts

echo "[E2E] 5b. Ensure pull consumers (remove any existing push consumers)..."
node --loader ts-node/esm scripts/ops/ensure-pull-consumers.ts 2>/dev/null || true

echo "[E2E] 5c. Seed governance E2E (state, drift, MASTER/MITL/YOLO proposals)..."
node --loader ts-node/esm scripts/ops/seed-governance-e2e.ts 2>/dev/null || true
...
node --loader ts-node/esm scripts/checks/verify-governance-paths.ts
