#!/usr/bin/env bash
# Full Liquid NLI v3 pipeline on Hugging Face Jobs — L4 GPU.
#
# Stages: MNLI probe (15k) → domain (1k corpus) → refine → push to Hub.
#
# Prerequisites:
#   hf auth login   (or export HF_TOKEN=hf_...)
#   HF Pro/Team/Enterprise
#
# Usage:
#   bash scripts/run-hf-l4-full-pipeline-v3.sh upload   # one-time: push script + dataset
#   bash scripts/run-hf-l4-full-pipeline-v3.sh submit   # launch job
#
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"

SCRIPT_PATH="model_evals/liquidai-encoders/hf_jobs/train_liquid_nli_v3_l4.py"
...
HF_HUB_MODEL="${HF_HUB_MODEL:-jeanbaptdzd/lfm25-nli-v3-calibrated-l4}"
HF_SCRIPT_URL="${HF_SCRIPT_URL:-https://huggingface.co/${HF_SCRIPT_REPO}/resolve/main/train_liquid_nli_v3_l4.py}"
...
upload_assets() {
  ...
  hf upload "$HF_SCRIPT_REPO" "$SCRIPT_PATH" train_liquid_nli_v3_l4.py \
    --commit-message "Liquid NLI v3 full pipeline L4 script"

  echo "== Upload domain dataset to ${HF_DATA_REPO} =="
...
submit_job() {
  echo "== HF Jobs: L4 full Liquid NLI v3 pipeline =="
  echo "Flavor: $FLAVOR  Timeout: $TIMEOUT"
  echo "Hub model: $HF_HUB_MODEL"
  ...
  echo "Logs:    hf jobs logs -f <job-id>"
  echo "Wait:    hf jobs wait <job-id>"
  echo ""
  echo "Pull final checkpoint locally:"
  echo "  hf download ${HF_HUB_MODEL} --local-dir workers/facts-worker/checkpoints/nli-domain-v3-calibrated"
  echo ""
  echo "Gold eval after pull:"
