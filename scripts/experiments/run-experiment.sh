...
#
# Usage: ./scripts/experiments/run-experiment.sh <exp_id> [options]
...
node --loader ts-node/esm scripts/ops/reset-e2e.ts 2>/dev/null || true

echo "[Exp] Ensuring schema, bucket, stream..."
node --loader ts-node/esm scripts/ops/ensure-schema.ts
node --loader ts-node/esm scripts/ops/ensure-bucket.ts
node --loader ts-node/esm scripts/ops/ensure-stream.ts
...
if [ "$RUN_SWARM" = 1 ]; then
  ...
  CHECK_FEED=1 node --loader ts-node/esm scripts/checks/check-services.ts 2>/dev/null || {
  ...
  if [ "$EXP_ID" = "exp4" ] || [ "$EXP_ID" = "exp5" ] || [ "$EXP_ID" = "noisy" ]; then
    ...
    node --loader ts-node/esm scripts/ops/simulate-mitl-approve.ts --finality-option=approve_finality &
    SIM_PID=$!
  fi
fi
...
run_single_experiment() {
  ...
  node --loader ts-node/esm scripts/experiments/drive-experiment.ts \
    --corpus="$corpus" \
    --rounds="$r" \
    --interval="$i" \
    --drain="$d" \
...
case "$EXP_ID" in
  ...
  exp7)
    ...
    for mode in YOLO MITL MASTER; do
      ...
      # ── Reset DB/NATS ──
      node --loader ts-node/esm scripts/ops/reset-e2e.ts 2>/dev/null || true
      node --loader ts-node/esm scripts/ops/ensure-schema.ts 2>/dev/null
      node --loader ts-node/esm scripts/ops/ensure-stream.ts 2>/dev/null
      ...
      if [ "$RUN_SWARM" = 1 ]; then
        ...
        if [ "$mode" = "MITL" ]; then
          ...
          node --loader ts-node/esm scripts/ops/simulate-mitl-approve.ts &
          SIM_PID=$!
        fi
      fi
      ...
      node --loader ts-node/esm scripts/experiments/collect-experiment-results.ts "exp7" 2>/dev/null || true
    done
    unset GOVERNANCE_MODE
    echo ""
    echo "[Exp] Exp7 complete. Analyzing tier coverage..."
    node --loader ts-node/esm scripts/experiments/analyze-tier-coverage.ts "docs/experiments/exp7/results/" 2>/dev/null || true
  ...
  exp8)
    ...
    for adv_mode in baseline inflate collude compensate; do
      ...
      node --loader ts-node/esm scripts/experiments/drive-exp8-adversarial.ts \
        --mode="$adv_mode" \
        --rounds="$ROUNDS" \
        --interval="$INTERVAL" \
        "${EXTRA_ARGS[@]}"
      ...
      ADVERSARIAL_MODE="$adv_mode" \
        node --loader ts-node/esm scripts/experiments/collect-experiment-results.ts "exp8" 2>/dev/null || true
    done
    unset GOVERNANCE_MODE
    echo ""
    echo "[Exp] Exp8 complete. Analyzing adversarial defense..."
    node --loader ts-node/esm scripts/experiments/analyze-adversarial-defense.ts "docs/experiments/exp8/results/" 2>/dev/null || true
  ...
  exp9)
    ...
    npx tsx scripts/experiments/drive-exp9-confluence.ts "${EXTRA_ARGS[@]}"
  ...
  exp5)
    ...
    for mode in YOLO MITL MASTER; do
      ...
      node --loader ts-node/esm scripts/experiments/collect-experiment-results.ts "exp5" 2>/dev/null || true
    done
    unset GOVERNANCE_MODE
  ...
  exp1-sweep)
    ...
    for C in 0 1 3 5; do
      ...
      node --loader ts-node/esm scripts/experiments/collect-experiment-results.ts "exp1" 2>/dev/null || true
    done
  ...
  exp-ab)
    ...
    node --loader ts-node/esm scripts/experiments/collect-experiment-results.ts "exp-ab" "$AB_DIR/scalar" 2>/dev/null || true
    ...
    node --loader ts-node/esm scripts/experiments/collect-experiment-results.ts "exp-ab" "$AB_DIR/vector" 2>/dev/null || true

    echo ""
    echo "[Exp] Exp-AB complete. Running analysis..."
    node --loader ts-node/esm scripts/experiments/analyze-scalar-vs-vector.ts "$AB_DIR" 2>/dev/null || true
  ...
  exp-skills)
    ...
    node --loader ts-node/esm scripts/experiments/collect-experiment-results.ts "exp-skills" "$BASELINE_DIR" 2>/dev/null || true
    ...
    node --loader ts-node/esm scripts/experiments/collect-experiment-results.ts "exp-skills" "$SKILLS_DIR_WITH" 2>/dev/null || true
...
node --loader ts-node/esm scripts/experiments/collect-experiment-results.ts "$EXP_ID"
