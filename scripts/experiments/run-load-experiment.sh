...
#
# Usage:
#   ./scripts/experiments/run-load-experiment.sh [OPTIONS]
#
# Options:
...
#
# Single run example:
#   ./scripts/experiments/run-load-experiment.sh --injection=stress --graph=large --duration=180
#
# Full sweep:
#   ./scripts/experiments/run-load-experiment.sh --sweep
#
set -e
cd "$(dirname "$0")/../.."
...
run_single() {
  ...
  node --loader ts-node/esm scripts/experiments/seed-exp2-graph.ts --claims="$CLAIMS" --rho="$RHO"
  ...
  LOAD_INJECT_DELAY_MS="$LOAD_INJECT_DELAY_MS" LOAD_BURST="$LOAD_BURST" \
    node --loader ts-node/esm scripts/experiments/loadgen-inject.ts
  ...
  EXP_ID=exp-load LOAD_RUN_ID="$run_id" \
    ...
    LOAD_DURATION="$DURATION" node --loader ts-node/esm scripts/experiments/collect-experiment-results.ts exp-load
