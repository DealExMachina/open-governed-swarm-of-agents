...
#
# Usage:
#   bash scripts/experiments/run-experiment-batch.sh <exp_id> <n_runs> [experiment options...]
#
# Examples:
#   bash scripts/experiments/run-experiment-batch.sh exp1 3 --contradictions=3
#   bash scripts/experiments/run-experiment-batch.sh exp3 3 --pattern=spike-and-drop --rounds=4
#   FINALITY_GATES_DISABLED=1 bash scripts/experiments/run-experiment-batch.sh exp3 3 --pattern=spike-and-drop
#
# Results:
...
for i in $(seq 1 "$N_RUNS"); do
  ...
  bash scripts/experiments/run-experiment.sh "$EXP_ID" "$@" 2>&1 | tee "${RUN_DIR}/run.log"
...
echo ""
echo "──── Analysis ────"
node --loader ts-node/esm scripts/experiments/analyze-experiment.ts "$BATCH_DIR" 2>&1 || echo "[Batch] Analysis script failed (may not exist yet)"
