...
const PROPAGATION_PATH =
  process.env.PROPAGATION_PATH ?? join(process.cwd(), "propagation.yaml");
    ...
    return {
      propagation: {
        ...
        diffusion: {
          ...
          max_alpha_ratio:
            parsed.propagation.diffusion?.max_alpha_ratio ?? 0.95,
          sheaf_mode:
            ((parsed.propagation.diffusion as Record<string, unknown>)
              ?.sheaf_mode as "constant" | "projection" | undefined) ??
            "constant",
        },
        admissible_set: {
          type: parsed.propagation.admissible_set?.type ?? "convex_box",
          support_range: parsed.propagation.admissible_set?.support_range ?? [
            0, 1,
          ],
          refutation_range: parsed.propagation.admissible_set
            ?.refutation_range ?? [0, 1],
        },
        noise: {
          epsilon_max_initial:
            parsed.propagation.noise?.epsilon_max_initial ?? 0.15,
          log_perturbation_norms:
            parsed.propagation.noise?.log_perturbation_norms ?? true,
        },
        iss: {
          max_contradiction_rate:
            parsed.propagation.iss?.max_contradiction_rate ?? 0.3,
          alert_on_violation:
            parsed.propagation.iss?.alert_on_violation ?? true,
        },
        topology_constraints: {
          min_spectral_gap:
            parsed.propagation.topology_constraints?.min_spectral_gap ?? 0.1,
          max_degree: parsed.propagation.topology_constraints?.max_degree ?? 4,
        },
        topology: parsed.propagation.topology
          ? {
              preset:
                (parsed.propagation.topology as TopologyConfig).preset ??
                "complete",
