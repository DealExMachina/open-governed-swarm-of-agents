...
| **R3** | Incremental `src/` domains | `src/studio/` + `src/finality/` with legacy barrels | **done** (this PR) |
| **R4** | Split Studio HTML/JS | Assets under `public/studio/` (#26 done); extract shell vs graph logic | Open (#29) |
...
Depends on assets living under `public/studio/` (done). Then extract:
