...
\begin{itemize}
  ...
  \item A product order $\mathcal{M} = \mathcal{L} \times \mathcal{A}$ that unifies governance admissibility and convergence rank into a single well-founded descent domain, with a deterministic reduction kernel guaranteeing epoch termination under $\delta_{\min}$-discretisation (Theorem~\ref{thm:termination}). See Remark~\ref{rem:algebraic-status} for the precise algebraic status.
...
The \emph{governance-convergence product order} is the Cartesian product $\mathcal{M} = \mathcal{L} \times \mathcal{A}$, ordered componentwise~\cite{davey2002}:
\[
...
\begin{remark}[Algebraic status of $\mathcal{M}$]
\label{rem:algebraic-status}
We use the term ``product order'' rather than ``product lattice'' deliberately. While $\mathcal{L}$ is a chain (hence a lattice) and $\mathcal{A} = [0,1]^k$ with the componentwise order is a complete lattice (with $\wedge = \min$, $\vee = \max$ per component), the implementation does \emph{not} use the join or meet operations on $\mathcal{M}$. The reduction kernel uses only the partial order (Definition~\ref{def:admissibility}) to test admissibility of transitions---it never computes the least upper bound or greatest lower bound of two governance states. Formally, $\mathcal{M}$ is a \emph{well-founded partially ordered set} (poset) used as a descent domain for epoch termination, not a lattice in the algebraic sense.

This paper establishes the product order as a practical tool for governance admissibility. Companion work extends the stack with graph-structured evidence coordination among roles (developed separately). A natural extension---currently under investigation---is to derive $\mathcal{M}$ as a \emph{concept lattice} in the sense of Formal Concept Analysis~\cite{ganter1999,wille1982}: a structure where agent contributions form the \emph{extent}, governance constraints form the \emph{intent}, and the lattice order emerges from the Galois connection between them. Such a construction would provide richer structure (formal concepts, attribute implications, concept navigation) than the current product order, and connect naturally to the audit, lineage, and constraint-satisfaction requirements of regulated environments. This formalization is the subject of planned future work.
\end{remark}
...
The architecture partially addresses this limitation through the product order $\mathcal{M} = \mathcal{L} \times \mathcal{A}$ (Definition~\ref{def:product-lattice}), where the convergence rank $\mathcal{A} = (\mathbb{R}_{\geq 0})^k$ tracks each dimension independently. The reduction kernel's admissibility check (Definition~\ref{def:admissibility}) evaluates per-dimension regression rather than relying solely on the scalar aggregate, preventing transitions that improve the aggregate while regressing individual dimensions. However, the finality evaluator's ultimate decision still relies on the scalar goal score and gate predicates. A fully per-dimension finality criterion---requiring each dimension to independently satisfy its target---remains a direction for future work.
