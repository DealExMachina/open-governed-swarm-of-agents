...
\begin{enumerate}
  ...
  \item \textbf{Dual-condition finality with layered gates and a perpetual lifecycle:} Finality spans two orthogonal layers. On the \emph{semantic layer}, vector finality $F^*$ requires every governance dimension to independently meet its threshold; monotonicity, evidence-coverage, oscillation, quiescence, and minimum-content gates guard against premature finality. On the \emph{propagation layer}, evidence diffuses on a cellular sheaf and the sheaf Dirichlet energy $f(x)$ certifies that the reasoning roles have reached a global section. A scope is RESOLVED only when both certify at once (the dual-condition $\wedge$-gate). The scalar disagreement function $V(t)$---a genuine Lyapunov function for the restricted CRDT system (Section~\ref{sec:convergence-guarantee})---is retained as an operational \emph{diagnostic} (rate, ETA, plateau) rather than the admissibility criterion. When autonomous convergence stalls, the system routes to structured human review. Finality is certified with Ed25519-signed JWS certificates---but finality is not terminal: new evidence, regulatory changes, or periodic review triggers re-open convergence over the same graph, producing a chain of certified checkpoints that models the perpetual lifecycle of regulated knowledge.
...
\begin{itemize}
  ...
  \item A formal, dual-layer convergence mechanism. On the semantic layer, a disagreement function $V(t)$ (Theorem~\ref{thm:lyapunov} establishes it as a genuine Lyapunov function for the restricted CRDT system) drives five independent gates (monotonicity, evidence coverage, oscillation detection, quiescence, minimum content), with vector finality $F^*$ as the non-compensable admissibility predicate and $V(t)$ reframed as a diagnostic certificate. On the propagation layer, inter-role agreement is modelled as diffusion on a cellular sheaf whose exact Lyapunov function---the sheaf Dirichlet energy $f(x) = x^\top L_{\mathcal{F}} x$---vanishes precisely on the global sections $H^0(G;\mathcal{F})$ (Theorem~\ref{thm:dirichlet-contraction}); the two layers combine under a dual-condition ($\wedge$) finality gate.
...
Classical consensus assumes every agent observes the same scalar state. Our reasoning roles instead observe \emph{different} subspaces of the evidence, so agreement must be measured relative to what each pair of roles actually shares---the setting of \emph{cellular sheaves}. Hansen and Ghrist~\cite{ghrist2022} develop a spectral theory in which the sheaf Laplacian $L_{\mathcal{F}} = \delta^\top\delta$ generalises the graph Laplacian, the Dirichlet energy $\|\delta x\|^2$ measures disagreement across restriction maps, and the kernel $H^0(G;\mathcal{F})$ is the space of \emph{global sections} (consistent assignments); Hansen and Ghrist~\cite{hansen2021opinion} instantiate this as opinion dynamics in which sheaf diffusion drives a network toward agreement on shared discourse. Our propagation layer (Section~\ref{sec:propagation-layer}) adopts this machinery directly: evidence diffuses along a role-graph sheaf, and finality on that layer is reaching a global section rather than crossing a scalar threshold.
...
Finality is a statement about a \emph{lattice}, not a scalar threshold, and it spans two orthogonal layers that must each reach the bottom of their respective order (Table~\ref{tab:two-layer}). The \emph{semantic layer} asks whether the aggregate governance dimensions meet their targets; its admissibility test is the vector finality predicate $F^*$ (Section~\ref{sec:vector-finality}), for which the scalar Lyapunov function $V(t)$ is a diagnostic certificate. The \emph{propagation layer} asks whether the reasoning roles actually agree on their shared observations; its admissibility test is reaching a global section of a cellular sheaf, certified by the sheaf Dirichlet energy $f(x)$ (Section~\ref{sec:propagation-layer}). A scope is RESOLVED only when both layers certify simultaneously---the dual-condition $\wedge$-gate (Section~\ref{sec:dual-condition}).
...
Theorem~\ref{thm:lyapunov} makes $V(t)$ a sound \emph{monitoring} instrument---its sign of change, exponential rate $\alpha$, ETA, and plateau EMA reveal whether the semantic layer is converging, stalled, or diverging, and drive routing to HITL or ESCALATED. It is deliberately \emph{not} the finality criterion: certifying RESOLVED on $V(t) \le \theta$ would re-expose the compensation attack of Section~\ref{sec:vector-finality}, since a scalar weighted sum lets a strong dimension mask a weak one. Admissibility is therefore the \emph{vector} predicate $F^*(t)$, conjoined with the propagation-layer condition of Section~\ref{sec:dual-condition}; $V(t)$ only describes the path taken to satisfy them.
\end{remark}
...
\]
the total squared disagreement across all sheaf edges, restricted to the subspaces the incident roles share.
\end{definition}
...
\midrule
RESOLVED & $F^*(t) \wedge [f(x_t) < \varepsilon_{\mathrm{prop}}]$: vector finality (gates A--E) and propagation global section & JWS certificate issued \\
...
\bottomrule
\end{tabular}
\caption{Finality states and their triggers. RESOLVED requires the dual-condition gate $F^*(t) \wedge [f(x_t) < \varepsilon_{\mathrm{prop}}]$:
semantic-layer vector finality $F^*(t)$ (all dimensions within threshold tolerance, per-dimension gates A$^d$/C$^d$, global gates B/D/E per Definition~\ref{def:vector-finality})
\emph{and} propagation-layer convergence to a global section (Section~\ref{sec:dual-condition}).
Upon RESOLVED, an Ed25519-signed finality certificate is emitted (Section~\ref{sec:certificates}).
The propagation condition defaults to true when disabled or when no propagation history exists; fallback to scalar finality $F(t)$ (Definition~\ref{def:finality}) is available when per-dimension finality is disabled.}
\label{tab:finality}
\end{table}
...
\begin{enumerate}
  ...
  \item \textbf{Propagation layer validated in isolation.} The sheaf Dirichlet energy $f(x)$ and the dual-condition gate (Section~\ref{sec:dual-condition}) are implemented and unit-tested, and Theorem~\ref{thm:dirichlet-contraction} follows from standard sheaf-diffusion spectral analysis; however, the end-to-end effect of the $\wedge$-gate on finality timing across the benchmark suite is not yet reported---the convergence experiments here characterise the semantic-layer $V(t)$. Robustness of $f$ under bounded evidence noise (input-to-state stability) and machine-checked versions of the contraction bound are deferred to future work.
