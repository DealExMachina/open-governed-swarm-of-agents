...
A product lattice $\mathcal{M} = \mathcal{L} \times \mathcal{A}$ with explicit meet and join unifies governance admissibility with a multi-dimensional convergence rank; the stalks carry Riesz-space structure making $V(t) = \|\mathbf{g}(t)\|^2_\mathbf{w}$ monotone in the order. An Epoch Termination Theorem guarantees finite stabilisation under $\delta_{\min}$-discretisation.
Convergence is tracked by this Lyapunov function; a \emph{vector finality} predicate prevents compensation attacks where one dimension masks another's failure.
...
The Lyapunov function $V(t) = \|\mathbf{g}(t)\|^2_\mathbf{w}$ is monotone in the lattice order; vector finality with per-dimension gates prevents both premature and compensated finality; Ed25519-signed certificates provide cryptographic non-repudiation within a perpetual checkpoint lifecycle.
