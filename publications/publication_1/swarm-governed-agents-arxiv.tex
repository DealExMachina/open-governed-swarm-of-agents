...
\title{Coordination of Governed Agents Over Shared Context:\\Declarative Governance and Dual-Condition Lattice Finality}
...
Finality is \emph{lattice-geometric} and spans two orthogonal layers, each of which must reach the bottom of its order before a scope is resolved.
On the \emph{semantic layer}, a product lattice $\mathcal{M} = \mathcal{L} \times \mathcal{A}$ with explicit meet and join unifies governance admissibility with a multi-dimensional convergence rank; the stalks carry Riesz-space structure, and a \emph{vector finality} predicate $F^*$ requires every governance dimension to independently meet its threshold, provably preventing compensation attacks where one dimension masks another's failure.
On the \emph{propagation layer}, ten reasoning roles coupled by a cellular sheaf run an evidence diffusion whose exact Lyapunov function is the \emph{sheaf Dirichlet energy} $f(x) = x^\top L_{\mathcal{F}}\, x = \|\delta x\|^2$; it contracts at rate $(1-\alpha\lambda_2)^2$ and vanishes precisely on the \emph{global sections} $H^0(G;\mathcal{F})$, the states where all connected roles agree on their shared observations.
A scope is RESOLVED under a \emph{dual-condition} ($\wedge$) gate---$F^*(t) \wedge [\,f(x) < \varepsilon_{\mathrm{prop}}\,]$---so that neither a compensated semantic score nor a premature consensus can certify alone.
The scalar $V(t) = \|\mathbf{g}(t)\|^2_\mathbf{w}$ is retained as a \emph{diagnostic} certificate (convergence rate, ETA, plateau, pressure), not the admissibility test; an Epoch Termination Theorem guarantees finite stabilisation under $\delta_{\min}$-discretisation, and Ed25519-signed certificates provide cryptographic non-repudiation.
...
A \emph{product lattice} $\mathcal{M} = \mathcal{L} \times \mathcal{A}$, with explicit meet and join on both factors, unifies governance levels with a multi-dimensional convergence rank; the stalks carry Riesz-space structure making $V$ a norm-monotone Lyapunov function.
A deterministic \emph{reduction kernel} evaluates every proposal against $\mathcal{M}$, producing Accept, Reject, or Escalate.
Finality is decided over two orthogonal layers. On the semantic layer, a \emph{vector finality} predicate $F^*$ with per-dimension gates prevents both premature and compensated finality; on the propagation layer, a cellular sheaf couples the reasoning roles and its \emph{Dirichlet energy} $f(x) = \|\delta x\|^2$ certifies that the roles have actually reached agreement (a global section) rather than merely producing a favourable aggregate score. A scope resolves only when both certify at once (the dual-condition $\wedge$-gate). A scalar Lyapunov disagreement function $V(t)$ is retained as an operational \emph{diagnostic}---convergence rate, ETA, plateau, and pressure---rather than the admissibility test.
...
\begin{enumerate}
  ...
  \item A product lattice $\mathcal{M} = \mathcal{L} \times \mathcal{A}$ with explicit meet and join, Riesz-space stalks, a deterministic reduction kernel, and an Epoch Termination Theorem guaranteeing finite convergence.
  \item A semantic-layer vector finality predicate that provably prevents compensation attacks (Theorem~\ref{thm:vector-finality-soundness}), with the scalar Lyapunov function $V(t)$ reframed as a convergence diagnostic rather than the admissibility criterion.
  \item A propagation layer that models inter-role evidence agreement as diffusion on a cellular sheaf, whose exact Lyapunov function---the sheaf Dirichlet energy $f(x) = x^\top L_{\mathcal{F}} x$---vanishes precisely on the global sections $H^0(G;\mathcal{F})$, combined with the semantic layer under a dual-condition ($\wedge$) finality gate (Theorem~\ref{thm:dirichlet-contraction}).
...
Where classical consensus assumes every agent observes the same scalar state, our reasoning roles observe \emph{different} subspaces of the evidence, so agreement must be measured relative to what each pair of roles actually shares. This is exactly the setting of \emph{cellular sheaves}: Hansen and Ghrist~\cite{ghrist2022} develop a spectral theory in which the sheaf Laplacian $L_{\mathcal{F}} = \delta^\top\delta$ generalises the graph Laplacian, the Dirichlet energy $\|\delta x\|^2$ measures disagreement across restriction maps, and its kernel $H^0(G;\mathcal{F})$ is the space of \emph{global sections} (consistent assignments). Hansen and Ghrist~\cite{hansen2021opinion} instantiate this as opinion dynamics, where sheaf diffusion drives a network toward agreement on shared discourse. Our propagation layer (Section~\ref{sec:propagation-layer}) adopts this machinery directly: evidence diffuses along a role-graph sheaf, and finality on this layer is reaching a global section rather than crossing a scalar threshold.
...
Our governance chain $\mathcal{L}$ is structurally simple; the contribution lies in the product lattice $\mathcal{M} = \mathcal{L} \times \mathcal{A}$ unifying governance admissibility with convergence rank, with the stalks $\mathcal{A} = [0,1]^k$ carrying Riesz-space structure~\cite{ghrist2022}: $V(t) = \|\mathbf{g}(t)\|^2_\mathbf{w}$ is a lattice norm monotone in the order, and evidence decomposes into non-overlapping positive/negative parts.
...
Formal Concept Analysis~\cite{ganter1999,wille1982} derives lattice structures from incidence relations; further specialisations of $\mathcal{M}$ remain exploratory.
...
The \emph{governance-convergence product lattice} is $\mathcal{M} = \mathcal{L} \times \mathcal{A}$, ordered componentwise~\cite{davey2002}:
$(\ell, \mathbf{a}) \leq_{\mathcal{M}} (\ell', \mathbf{a}') \iff \ell \leq_{\mathcal{L}} \ell' \wedge \mathbf{a} \leq_{\mathcal{A}} \mathbf{a}'$,
with meet $(\ell,\mathbf{a}) \wedge (\ell',\mathbf{a}') = (\ell \wedge_{\mathcal{L}} \ell',\, \mathbf{a} \wedge_{\mathcal{A}} \mathbf{a}')$ and join defined dually.
\end{definition}

\begin{remark}[Algebraic status]
\label{rem:algebraic-status}
$\mathcal{M}$ is a \emph{complete distributive lattice}. $\mathcal{L}$ (chain MASTER~$\leq$~MITL~$\leq$~YOLO) and $\mathcal{A} = [0,1]^k$ (componentwise order, $\wedge = \min$, $\vee = \max$) both carry explicit meet and join, implemented as \texttt{GovernanceLevel::meet/join}, \texttt{ConvergenceRank::meet/join}, \texttt{LatticePoint::meet/join}. The stalks $\mathcal{A}$ further form a \emph{Riesz space} (vector lattice): $V(t) = \|\mathbf{g}(t)\|^2_\mathbf{w}$ is the squared weighted norm of the gap vector $\mathbf{g}(t) = (\tau - \mathbf{a}(t))^+$, monotone decreasing in the lattice order. Evidence decomposes into non-overlapping positive/negative parts; the drift chain $\mathcal{D}$ couples to $\mathcal{L}$ via an anti-monotone map (higher drift forces more restrictive governance), making the stabilising feedback algebraically explicit.
\end{remark}
...
Finality is a statement about a \emph{lattice}, not a scalar threshold, and it spans two orthogonal layers that must each reach the bottom of their respective order (Table~\ref{tab:two-layer}). The \emph{semantic layer} asks whether the aggregate governance dimensions meet their targets; its admissibility test is the vector finality predicate $F^*$ (Section~\ref{sec:vector-finality}), and the scalar Lyapunov function $V(t)$ is a diagnostic certificate over it. The \emph{propagation layer} asks whether the reasoning roles actually agree on their shared observations; its admissibility test is reaching a global section of a cellular sheaf, certified by the sheaf Dirichlet energy $f(x)$ (Section~\ref{sec:propagation-layer}). A scope is RESOLVED only when both layers certify simultaneously---the dual-condition $\wedge$-gate (Section~\ref{sec:dual-condition}).
...
\toprule
 & \textbf{Semantic layer} & \textbf{Propagation layer} \\
\midrule
State & dimension score vector $\mu(t) \in [0,1]^k$ & evidence state $x \in \mathbb{R}^{2D\cdot N}$ (support/refutation per role per dimension) \\
Order & convergence-rank lattice $\mathcal{M} = \mathcal{L}\times\mathcal{A}$ & bilattice stalks over the role graph, coupled by a cellular sheaf \\
Admissibility & vector finality $F^*(t)$: gap $(\tau-\mu)^+$ zero on every required dimension (non-compensable) & global section: $f(x) = x^\top L_{\mathcal{F}} x < \varepsilon_{\mathrm{prop}}$ \\
Lyapunov role & $V(t)$ --- a \emph{diagnostic} scalar (rate, ETA, plateau) & $f(x)$ --- the \emph{exact} Lyapunov function of the diffusion \\
\bottomrule
\end{tabular}
\caption{The two convergence layers. Admissibility for RESOLVED is $F^*(t) \wedge [f(x) < \varepsilon_{\mathrm{prop}}]$; the scalar $V(t)$ certifies \emph{how} the semantic layer is moving, not \emph{whether} it is admissible.}
\label{tab:two-layer}
\end{table}
...
\begin{remark}[$V(t)$ is a diagnostic, not the admissibility test]
\label{rem:v-diagnostic}
Theorem~\ref{thm:lyapunov} makes $V(t)$ a sound \emph{monitoring} instrument: its sign of change, exponential rate $\alpha$, ETA, and plateau EMA tell operators whether the semantic layer is converging, stalled, or diverging, and drive routing to HITL or ESCALATED. It is deliberately \emph{not} the finality criterion. Certifying RESOLVED on $V(t) \le \theta$ would re-expose the compensation attack of Section~\ref{sec:vector-finality}, since a scalar weighted sum lets a strong dimension mask a weak one. Admissibility is therefore the \emph{vector} predicate $F^*(t)$, conjoined with the propagation-layer condition below; $V(t)$ only describes the path taken to satisfy them.
\end{remark}
...
\subsection{Propagation Layer: Sheaf Dirichlet Energy and Global Sections}
\label{sec:propagation-layer}

Vector finality certifies that the aggregate scores meet their targets. It does not certify that the reasoning roles that produced those scores actually \emph{agree}: a favourable average can hide two roles that read the same evidence oppositely. The propagation layer closes this gap.

Let $G = (R, E)$ be the graph of $|R| = N$ reasoning roles. Each role $i$ holds an evidence vector $x_i \in \mathbb{R}^{2D}$ (support and refutation for each of $D$ dimensions), so the global state is $x \in \mathbb{R}^{2D \cdot N}$. The roles are coupled by a \emph{cellular sheaf} $\mathcal{F}$ over $G$: each edge $e = (i,j)$ carries restriction maps $\mathcal{F}_{i\trianglelefteq e}, \mathcal{F}_{j\trianglelefteq e}$ onto a shared edge stalk, encoding \emph{which dimensions the two roles jointly observe}. The coboundary $\delta$ assembles the per-edge discrepancies $\delta_e x = \mathcal{F}_{i\trianglelefteq e} x_i - \mathcal{F}_{j\trianglelefteq e} x_j$, and the \emph{sheaf Laplacian} is $L_{\mathcal{F}} = \delta^\top \delta$~\cite{ghrist2022}. At each cycle the propagation agent runs one diffusion step
\[
x_{t+1} = \Pi_{\mathcal{A}}\!\left[(I - \alpha L_{\mathcal{F}})\, x_t + \varepsilon_t\right],
\]
where $\alpha > 0$ is the step size, $\varepsilon_t$ is fresh evidence, and $\Pi_{\mathcal{A}}$ projects back onto the admissible box.

\begin{definition}[Sheaf Dirichlet Energy]
\label{def:dirichlet}
The Dirichlet energy of an evidence state $x$ is
\[
f(x) = x^\top L_{\mathcal{F}}\, x = \|\delta x\|^2 = \sum_{e \in E} \|\delta_e x\|^2 \; \geq \; 0 .
\]
It is the total squared disagreement across all sheaf edges, restricted to the subspaces the incident roles share.
\end{definition}

\begin{theorem}[Contraction and Global Sections]
\label{thm:dirichlet-contraction}
For $0 < \alpha < 2/\lambda_{\max}(L_{\mathcal{F}})$, the noise-free diffusion $x_{t+1} = (I - \alpha L_{\mathcal{F}}) x_t$ is the gradient flow of $f$ and satisfies, on the orthogonal complement of $\ker L_{\mathcal{F}}$,
\[
f(x_{t+1}) \leq (1 - \alpha \lambda_2)^2\, f(x_t),
\]
where $\lambda_2$ is the smallest nonzero eigenvalue (spectral gap) of $L_{\mathcal{F}}$. Moreover $f(x) = 0$ iff $x \in \ker \delta = H^0(G;\mathcal{F})$, the space of \emph{global sections}: assignments on which every pair of connected roles agrees over their shared observed subspace.
\end{theorem}

\begin{proof}[Proof sketch]
$f(x) = x^\top L_{\mathcal{F}} x$ with $L_{\mathcal{F}} \succeq 0$, and $\nabla f = 2 L_{\mathcal{F}} x$, so $x_{t+1} = x_t - \tfrac{\alpha}{2}\nabla f(x_t)$ is gradient descent on $f$. Diagonalising $L_{\mathcal{F}}$, each eigencomponent with eigenvalue $\lambda$ is scaled by $(1-\alpha\lambda)$ per step; components in $\ker L_{\mathcal{F}}$ are fixed, and the slowest-decaying nonzero mode has factor $(1-\alpha\lambda_2)$, giving the stated rate on $f = \|L_{\mathcal{F}}^{1/2} x\|^2$. The kernel characterisation $\ker L_{\mathcal{F}} = \ker \delta = H^0(G;\mathcal{F})$ is standard for the sheaf Laplacian~\cite{ghrist2022}.
\end{proof}

The propagation-layer analogue of ``all dimensions reach target'' is thus ``$f(x) = 0$'', i.e.\ reaching a global section. In practice we test $f(x) < \varepsilon_{\mathrm{prop}}$ for a small tolerance.

\begin{remark}[Why not the variance proxy]
\label{rem:omega-proxy}
A tempting shortcut is the variance proxy $\Omega(x) = \sum_i \|x_i - \bar{x}\|^2$ (distance from the mean). This equals $f$ only on the constant complete sheaf, where $f(x) = N\,\Omega(x)$. On a \emph{projection} sheaf---the production default, where roles observe different dimensions---$\Omega$ counts disagreement on dimensions a role never observes and can stay strictly positive even after all \emph{reachable} disagreement has vanished (two roles observing disjoint dimensions have $f \to 0$ while $\Omega > 0$). Gating finality on $\Omega$ there makes RESOLVED either unreachable or spurious; the engine therefore computes $f$ directly from the Laplacian it already materialises, retaining $\Omega$ only as a topology-health signal and exposing the per-edge decomposition $\|\delta_e x\|^2$ for bottleneck attribution.
\end{remark}

\subsection{Dual-Condition Finality}
\label{sec:dual-condition}

The two layers are independent and defend against distinct failure modes. Vector finality $F^*$ blocks the \emph{compensation attack} (a weak dimension hidden behind a strong one); the Dirichlet condition blocks \emph{premature consensus} (declaring done while roles still structurally disagree). Neither subsumes the other, so RESOLVED requires both to certify simultaneously---the dual-condition ($\wedge$) gate:
\[
\textsc{Resolved}(t) \;\iff\; F^*(t) \;\wedge\; \bigl[\, f(x_t) < \varepsilon_{\mathrm{prop}} \,\bigr].
\]

\begin{remark}[Backward compatibility]
The propagation condition is additive and guarded. When the Dirichlet gate is disabled, or when no propagation history exists for a scope, $[f(x_t) < \varepsilon_{\mathrm{prop}}]$ defaults to true and the gate reduces to the semantic layer $F^*(t)$ alone. When $F^*$ holds but the propagation condition does not, the evaluator emits a \texttt{dirichlet\_hold} signal---semantics ready, roles still converging---for observability without falsely resolving.
\end{remark}
...
\midrule
RESOLVED & $F^*(t) \wedge [f(x_t) < \varepsilon_{\mathrm{prop}}]$: vector finality (gates A--E) and propagation global section & JWS certificate \\
...
\begin{enumerate}
  ...
  \item \textbf{Propagation layer validated in isolation.} The sheaf Dirichlet energy $f(x)$ and the dual-condition gate are implemented and unit-tested, and Theorem~\ref{thm:dirichlet-contraction} follows from standard sheaf-diffusion spectral analysis; however, the end-to-end effect of the $\wedge$-gate on finality timing across the benchmark suite is not yet reported. The convergence experiments below characterise the semantic-layer $V(t)$. Robustness of $f$ under bounded evidence noise (input-to-state stability) and machine-checked versions of the contraction bound are deferred to future work.
...
The product lattice $\mathcal{M} = \mathcal{L} \times \mathcal{A}$, with explicit meet and join on both factors and Riesz-space stalks, jointly tracks governance strictness and convergence progress in a well-founded ordering guaranteeing finite stabilisation.
Finality is lattice-geometric and dual-condition: the semantic layer certifies non-compensable vector finality $F^*$, while the propagation layer certifies that the reasoning roles have reached a global section of a cellular sheaf via the Dirichlet energy $f(x) = x^\top L_{\mathcal{F}} x$, and RESOLVED requires both at once. The scalar Lyapunov function $V(t) = \|\mathbf{g}(t)\|^2_\mathbf{w}$, monotone in the lattice order, is retained as a convergence diagnostic rather than the admissibility test; Ed25519-signed certificates provide cryptographic non-repudiation within a perpetual checkpoint lifecycle.
