 ...
 */
beforeAll(() => {
  process.env.DATABASE_URL = process.env.TEST_DATABASE_URL || "postgres://localhost/swarm_test";
...
export function createMockLogger() {
  ...
  return {
    debug: (msg: string, extra?: unknown) => logs.push({ level: "debug", msg, extra }),
    info: (msg: string, extra?: unknown) => logs.push({ level: "info", msg, extra }),
    warn: (msg: string, extra?: unknown) => logs.push({ level: "warn", msg, extra }),
    error: (msg: string, extra?: unknown) => logs.push({ level: "error", msg, extra }),
    logs,
    clear: () => logs.splice(0),
  };
}
