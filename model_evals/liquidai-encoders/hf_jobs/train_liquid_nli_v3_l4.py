...
# ]
# ///
"""Full Liquid NLI v3 pipeline on Hugging Face Jobs (L4).
...
Submit via scripts/run-hf-l4-full-pipeline-v3.sh
"""
...
def run_probe(model_id: str, device: str, out_dir: Path, max_samples: int, batch: int, grad_accum: int):
    ...
    save_nli_checkpoint(out_dir, model, tokenizer, meta, extra={"stage": "mnli-probe-v3-l4", "metrics": metrics})
    return out_dir
...
def run_domain(model_id: str, device: str, probe_dir: Path, train_path: Path, eval_path: Path, out_dir: Path, batch: int, grad_accum: int):
    ...
    snli_rows = load_snli_subset(2000, seed=42)
    train_mixed = mix_datasets(train_domain, snli_rows, 0.15, 42)
    ...
    save_nli_checkpoint(out_dir, model, tokenizer, meta, extra={"stage": "domain-v3-liquid-l4", "metrics": metrics})
    return out_dir
...
def run_refine(model_id: str, device: str, domain_dir: Path, train_path: Path, eval_path: Path, out_dir: Path, batch: int, grad_accum: int):
    ...
    save_nli_checkpoint(out_dir, model, tokenizer, meta, extra={"stage": "refine-v3-calibrated-l4", "metrics": metrics})
    (out_dir / "train_meta.json").write_text(
        json.dumps({"pipeline": "liquid-nli-v3-l4", "metrics": metrics}, indent=2) + "\n",
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    work = Path("/tmp/liquid-nli-v3-l4")
    ...
    if hub_model_id:
        ...
        readme = (
            f"# LFM2.5-Encoder-230M NLI v3 (Liquid recipe, L4)\n\n"
            f"Pipeline: MNLI probe → domain (wd=0.1) → refine (wd=0.01)\n\n"
