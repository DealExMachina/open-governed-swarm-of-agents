...
use sgrs_core::propagation::{
    compute_disagreement, contradiction_count, extract_contradictions, per_dimension_disagreement,
    propagation_step, spectral_analysis, AdmissibleProjection, CellularSheaf, ContradictionChannel,
...
fn bilattice_consensus_zero_disagreement() {
    ...
    assert!(
        ...
        "consensus should have 0 contradictions"
    );
...
fn bilattice_ignorance_zero_disagreement() {
    ...
    let state = EvidenceState {
        role_states: vec![
            EvidenceVector {
                support: vec![0.05, 0.08],
                refutation: vec![0.03, 0.07]
            };
            n
        ],
        num_roles: n,
    ...
    assert!(
        omega < 1e-12,
        "shared ignorance should have Ω ≈ 0, got {}",
        omega
    );
    assert!(
        contradictions.is_empty(),
        "shared ignorance has no contradictions"
    );
...
fn bilattice_contradiction_detected() {
    ...
    // Classify by channel
    let support_contradictions: Vec<_> = contradictions
        .iter()
        .filter(|c| c.channel == ContradictionChannel::Support)
        .collect();
    let refutation_contradictions: Vec<_> = contradictions
        .iter()
        .filter(|c| c.channel == ContradictionChannel::Refutation)
        .collect();

    println!(
        "  support channel: {} contradictions",
        support_contradictions.len()
    );
    println!(
        "  refutation channel: {} contradictions",
        refutation_contradictions.len()
    );
    ...
    for c in &contradictions {
        ...
        assert_eq!(
            c.dimension, 0,
            "contradictions should be on dim 0 (dim 1 agrees)"
        );
    }

    // Both channels should show contradictions on dim 0
    assert!(
        !support_contradictions.is_empty(),
        "support channel should have contradictions"
    );
    assert!(
        !refutation_contradictions.is_empty(),
        "refutation channel should have contradictions"
    );
    ...
    assert!(
        per_dim[0] > per_dim[1] * 10.0,
        "dim 0 should dominate disagreement"
    );
...
fn bilattice_partial_knowledge() {
    ...
    let state = EvidenceState {
        role_states: vec![
            ...
            EvidenceVector {
                support: vec![0.9, 0.5, 0.4],
                refutation: vec![0.1, 0.3, 0.3],
            },
            EvidenceVector {
                support: vec![0.85, 0.5, 0.45],
                refutation: vec![0.15, 0.3, 0.35],
            },
            // Roles 2,3: ignorant on dim 0 (low support, low refutation)
            EvidenceVector {
                support: vec![0.1, 0.5, 0.42],
                refutation: vec![0.1, 0.3, 0.32],
            },
            EvidenceVector {
                support: vec![0.12, 0.5, 0.43],
                refutation: vec![0.08, 0.3, 0.33],
            },
        ],
        num_roles: n,
        num_dims,
    ...
    println!("  Ω = {:.6}", omega);
    println!(
        "  per-dim: dim0={:.6}, dim1={:.6}, dim2={:.6}",
        per_dim[0], per_dim[1], per_dim[2]
    );

    assert!(omega > 0.0, "partial knowledge should have Ω > 0");
    assert!(
        per_dim[0] > per_dim[1],
        "dim 0 (partial info) should have highest disagreement"
    );
...
fn bilattice_channel_correctness() {
    ...
    for c in &c_support {
        println!(
            "    dim {} {:?} magnitude={:.2}",
            c.dimension, c.channel, c.magnitude
        );
    }
    assert_eq!(c_support.len(), 1, "should have exactly 1 contradiction");
    assert_eq!(
        c_support[0].channel,
        ContradictionChannel::Support,
        "should be support channel"
    );
    ...
    assert_eq!(
        c_refutation[0].channel,
        ContradictionChannel::Refutation,
        "should be refutation channel"
    );
    ...
    assert_eq!(
        c_both.len(),
        2,
        "should have 2 contradictions (one per channel)"
    );
    let channels: Vec<_> = c_both.iter().map(|c| c.channel).collect();
    assert!(
        channels.contains(&ContradictionChannel::Support),
        "should include support"
    );
    assert!(
        channels.contains(&ContradictionChannel::Refutation),
        "should include refutation"
    );
