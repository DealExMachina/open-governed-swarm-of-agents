...
\subsection{Development Roadmap: Formal Hardening}

The immediate development priority is \emph{formal hardening}: aligning the finality semantics with the lattice semantics that governance already uses, and systematically validating the assumptions that underpin the paper's formal claims.

\textbf{Per-dimension finality.} While the product lattice $\mathcal{M} = \mathcal{L} \times \mathcal{A}$ tracks convergence per-dimension during governance evaluation (the reduction kernel rejects transitions that regress any single dimension), the finality evaluator's ultimate resolution decision still relies on a scalar goal score. The planned extension replaces the scalar finality criterion with a vector predicate:
\[
F^*(t) = \bigwedge_d \bigl[e_d(t) \leq \epsilon_d \;\wedge\; G_A^{(d)} \;\wedge\; G_C^{(d)}\bigr] \;\wedge\; G_B \;\wedge\; G_D \;\wedge\; G_E
\]
where $e_d(t) = \max(0,\, \tau_d - \mu_d(t))$ is the per-dimension gap and $\tau_d$ the per-dimension threshold. This prevents compensation artifacts where one dimension over-improves while another remains unsafe---the non-scalar convergence criterion that SECP~\cite{delachica2026} identifies as essential.

\textbf{Assumption matrix.} Seven assumptions (A1--A7) have been identified that underpin the formal claims: well-founded descent of $\mathcal{A}$ (A1), kernel determinism (A2), monotone CRDT evolution (A3), progress precondition (A4), local confluence boundary (A5), non-compensability (A6), and tier completeness (A7). Each will be classified as \emph{proven}, \emph{empirically validated}, \emph{partially validated}, or \emph{open}, with explicit experimental evidence linking to the classification.

\textbf{Proof obligations.} Four proof obligations are planned: (PO-1)~epoch termination theorem with formal rank function in assistant-checkable form; (PO-2)~per-dimension finality soundness, demonstrating a scalar-pass/vector-fail counterexample and proving the vector criterion blocks it; (PO-3)~eventual consistency safety, bounding recovery after complete re-extraction; (PO-4)~routing completeness, constructing workloads that force Tier~1/2/3 path diversity.

\textbf{Extended experiment program.} Five new experiments are planned: (E1)~scalar vs vector finality on paired corpora, measuring false finality rate and blocked compensation cases; (E2)~Tier-3 reachability via deterministic-vs-LLM disagreement corpus; (E3)~discretization sweep over $\epsilon$ sensitivity; (E4)~confluence boundary extension beyond Exp.~9; (E5)~adversarial compensation attack comparing scalar vs vector finality defenses. Experiments will report confidence intervals and effect sizes, addressing the point-estimate limitation of the current results.
