...
\begin{enumerate}
  ...
  \item \textbf{Local confluence.} The connection to rewriting theory invokes Newman's Lemma, which requires local confluence as a precondition. Local confluence---that any two admissible transitions from the same state produce joinable results---is neither proven nor tested. The system provides kernel determinism (each proposal evaluation is deterministic) but not commutativity of independent transitions. The claimed ``partial confluence'' is a weaker property: only transitions certified compatible by the kernel are guaranteed to commute. Full confluence remains an open question.
  ...
  \item \textbf{Tier 2/3 governance routing.} The three-tier governance architecture is implemented and instrumented, but only Tier~1 (deterministic evaluation) was exercised in all experiments. The oversight LLM (Tier~2) consistently agreed with the deterministic result. No experiment triggered Tier~3 (full LLM governance). The three-tier architecture is therefore validated as implemented but not as exercised: the system's behavior under genuine Tier~2/3 disagreement is unknown.

  \item \textbf{Cooperative agent model.} All formal guarantees assume cooperative agents (Threat Model, Section~\ref{sec:problem-setting}). The monotonic confidence ratchet provides partial robustness against downward manipulation, but no defense exists against upward inflation (an adversarial agent that inflates confidence scores can drive $V(t)$ toward 0 without genuine resolution). Byzantine fault tolerance remains entirely unaddressed.
\end{enumerate}
