...
Three governance modes were run sequentially (YOLO, MITL, MASTER) on the same 7-document corpus, with database reset between modes.

In YOLO mode, the system processed all documents through the pipeline with epoch advancing to 21 and 24 decision records produced. In MITL mode (with simulated human approvals), a similar progression occurred with automatic finality review approval. In MASTER mode, the system produced epoch = 0 throughout---all proposals were blocked by the deterministic governance policy. No state transitions occurred and no decision records were generated.

This confirms the paper's core prediction: YOLO maximizes coverage (all documents processed, all proposals evaluated), MASTER maximizes autonomy (every drift signal blocks advancement), and MITL occupies an intermediate position. The convergence rate $\alpha$ could not be meaningfully compared across modes because MASTER produced no convergence points.
...
\paragraph{3. Governance mode affects coverage (confirmed).} Exp.~5 demonstrates that MASTER mode blocks all proposals (epoch = 0, zero decisions), YOLO processes all documents (epoch = 21, 24 decisions), and MITL occupies an intermediate position. This maps directly to the coverage--autonomy trade-off described in the paper and aligns with SECP's finding that stricter evaluation reduces coverage.
...
Per-dimension analysis confirms a structural cause: the scalar $V(t)$ oscillates between two values (0.250 and 0.550) because only the \texttt{contradiction\_resolution} dimension changes; \texttt{goal\_completion} is permanently stalled at 0.00, and two other dimensions are saturated at 1.00. The scalar metric collapses this into apparent oscillation, when the underlying dynamics are: one dimension flipping, one stalled, two at target. A non-scalar convergence criterion---requiring per-dimension progress rather than aggregate threshold crossing---would expose these dynamics directly and is the primary theoretical direction for future work.
...
\paragraph{Non-scalar convergence (v2).} The scalar $V(t)$ collapses per-dimension dynamics into misleading oscillation. The data shows that \texttt{goal\_completion} was stalled at 0.00 for all 50 evaluation points while \texttt{contradiction\_resolution} oscillated between 0 and 1---a pattern invisible to the scalar metric. A per-dimension convergence criterion would: (a) require each dimension to independently satisfy its target rather than relying on aggregate threshold crossing; (b) expose stalled dimensions as first-class diagnostic signals; (c) provide the per-dimension $\alpha$ needed for meaningful ETA estimation; (d) align with SECP's non-scalar coordination, where Pareto filtering and constructive objections prevent the collapse of structured disagreement. This is the primary theoretical direction for a v2 architecture.
...
The empirical claims are narrow and intentionally circumscribed (Sections~\ref{sec:boundaries} and \ref{sec:discussion}). Results from $n = 3$ repeated runs demonstrate reproducibility of $V(t)$ response to contradictions and gate mechanism effectiveness, but are not statistically powered distributional claims. The convergence guarantee (Proposition~\ref{prop:monotonic}) holds formally but was not demonstrated end-to-end: $V(t)$ never decreased to zero because no experiment produced full contradiction resolution. The gate ablation confirms that gates prevent false finality on adversarial input. No production-scale validation ($N > 50$ documents) and no multi-domain validation have been performed.
