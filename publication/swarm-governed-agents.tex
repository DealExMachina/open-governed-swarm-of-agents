...
Validation on two scenarios---M\&A due diligence (Project Horizon, $n = 3$ per condition) and multi-period financial consolidation with bitemporal restatement---demonstrates reproducible $V(t)$ tracking of disagreement (0.25~$\to$~0.55 on contradiction arrival, with spike-and-recovery dynamics confirmed across both domains), five-gate prevention of false finality (ablation-confirmed), governance modes producing distinct coverage--autonomy positions, and dual temporality correctly handling temporal restatements. The architecture is validated by 121 Rust and 305 TypeScript tests.
...
\begin{itemize}
  ...
  \item Validation on two scenarios---M\&A due diligence and financial consolidation with dual temporality---with 121 Rust and 305 TypeScript tests, demonstrating convergence, contradiction resolution, temporal restatement handling, and structured human review.
\end{itemize}
...
\midrule
Baseline        & 0.25 & 1--20  & Subsidiary filings ingested \\
Spike           & 0.55 & 21--22 & Alpha Q1 restatement triggers contradiction \\
Recovery        & 0.25 & 23     & Q2 document resolves with restated figures \\
Stable          & 0.25 & 24--42 & Auditor review + management response \\
\bottomrule
\end{tabular}
\caption{Financial consolidation $V(t)$ trajectory. The restatement spike (+120\%) and single-cycle recovery match the M\&A scenario dynamics, confirming domain-independent convergence behavior.}
\label{tab:financial-results}
\end{table}

The temporal restatement demonstrates dual temporality in action: Alpha's restated Q1 revenue (EUR~126.6M vs original EUR~127.4M) is recorded with transaction time May~22, superseding the April~14 original for the same valid period (Q1~2025). The system sets \texttt{superseded\_at} on original nodes. Gate~A (monotonicity) and Gate~B (evidence) correctly detect the inconsistency at epochs~21--22, escalating from HITL to ESCALATED. Recovery occurs within one extraction cycle when the Q2 document provides consistent restated comparatives.

Three properties distinguish this scenario: (1)~contradiction detection respects valid-time overlap---Q1 figures contradict only other Q1 figures, not Q2; (2)~the auditor's range-based observations (39--41\% gross margin) are correctly classified as compatible, avoiding false contradictions; (3)~the $V(t)$ spike-and-recovery pattern (0.25~$\to$~0.55~$\to$~0.25) matches the M\&A dynamics quantitatively, confirming that convergence behavior is domain-independent. The final semantic graph contains 5 active claims (restated figures), 4 active risks, and zero unresolved contradictions across 42~epochs.
...
\begin{enumerate}
  \item \textbf{Architectural feasibility.} Declarative governance over shared immutable context is implementable with current technology (Postgres, NATS, LLMs, OpenFGA, native Rust via napi-rs) and produces a functioning coordination system with 121 Rust unit tests, 305 TypeScript tests, and 7 convergence benchmarks.
...
This paper presents a paradigm for autonomous agent coordination based on declarative governance over shared immutable context. The core contributions are: (i)~a Lyapunov disagreement function with five independent gates providing layered guarantees against premature finality; (ii)~a bitemporal CRDT-inspired semantic graph with monotonicity invariants; (iii)~a product lattice $\mathcal{M} = \mathcal{L} \times \mathcal{A}$ with an Epoch Termination Theorem guaranteeing finite convergence via well-founded descent; (iv)~a deterministic Rust reduction kernel ensuring auditable, replayable governance independent of LLM stochasticity; (v)~perpetual finality with Ed25519-signed certificate chains; (vi)~three-tier governance routing with LLM-free fallback; and (vii)~validation on two domains---M\&A due diligence ($n = 3$) and financial consolidation with bitemporal restatement---confirming domain-independent convergence dynamics, validated by 121 Rust and 305 TypeScript tests.
