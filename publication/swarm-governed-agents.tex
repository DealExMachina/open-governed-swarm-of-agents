...
Validation on a realistic M\&A due-diligence scenario (Project Horizon) with repeated experiments ($n = 3$ per condition) demonstrates that $V(t)$ reproducibly tracks disagreement (rising from $0.251 \pm 0.001$ to $0.551 \pm 0.001$ on contradiction arrival), the five-gate mechanism prevents false finality on adversarial spike-and-drop input (confirmed by ablation: gates-disabled ESCALATED rate drops from 7\,\% to 0\,\%), and governance modes produce distinct positions on the coverage--autonomy spectrum: YOLO processes all proposals, MASTER evaluates every proposal through the full reduction kernel (rejecting on policy blocks and lattice incomparability), and MITL routes to human review. The architecture is validated by 121 Rust tests and 288 TypeScript tests across 37 suites.
...
\begin{itemize}
  ...
  \item Validation on a realistic M\&A due-diligence scenario with 121 Rust and 305 TypeScript tests, demonstrating convergence, contradiction resolution, and structured human review.
\end{itemize}
...
The XACML standard \cite{xacml2013} defines combining algorithms for multi-policy evaluation (deny-overrides, first-applicable). Pang et al. \cite{pang2019} formalized Zanzibar-style relationship-based access control, implemented in OpenFGA. Our v2 architecture replaces external policy engines with a native Rust \emph{reduction kernel} compiled via napi-rs: all policy evaluation, lattice admissibility checking, and mode routing execute as deterministic Rust code within the Node.js process, eliminating the overhead and operational complexity of external policy runtimes while preserving XACML combining algorithms for multi-policy resolution and OpenFGA for fine-grained governance of document access, transition approval, and policy modification.
...
\paragraph{Empirical status of the corollary.} The precondition---``every governance cycle produces strict progress on some dimension''---was \emph{not satisfied} in any experiment. In Exp.~1, the \texttt{goal\_completion} dimension was permanently stalled at 0.00 (no goal-resolution agent exists), and $V(t)$ never reached 0. The corollary is therefore formally correct but practically vacuous under the current agent ensemble. Satisfying it requires either an autonomous resolver agent that closes the convergence loop, or relaxing the precondition to require progress on at least one \emph{non-stalled} dimension.
...
Three independent runs ($n = 3$) with $c = 5$ contradicting documents from the Project Horizon M\&A corpus (7 documents total, including 2 resolution documents). Progressive resolution was injected at rounds 5, 6, and 7---each round marking up to 3 contradiction nodes as resolved and creating resolves edges for unresolved contradiction pairs.
...
\toprule
\textbf{Epoch} & \textbf{Score} & \textbf{$\pm$ std} & \textbf{$V(t)$} & \textbf{$\pm$ std} & \textbf{$n$} & \textbf{Phase} \\
\midrule
1--3   & 0.744 & 0.010 & 0.250 & 0.001 & 3 & Pre-contradiction \\
4--5   & 0.544 & 0.163 & 0.450 & 0.173 & 3 & Contradiction arrival \\
6--13  & 0.441 & 0.016 & 0.551 & 0.001 & 3 & Plateau (unresolved) \\
14--16 & 0.641 & 0.166 & 0.351 & 0.172 & 3 & Post-resolution recovery \\
17--21 & 0.541 & 0.182 & 0.451 & 0.174 & 3 & Re-contradiction (new docs) \\
\bottomrule
\end{tabular}
\caption{Exp.~1 convergence trajectory ($c = 5$, $n = 3$, progressive resolution at rounds 5--7). $V(t)$ rises on contradiction arrival, then \emph{drops from 0.551 to 0.351} when resolutions are injected, before rising again as later documents introduce new contradictions.}
\label{tab:exp1-results}
\end{table}

The trajectory exhibits three distinct phases: (1) $V(t)$ rises from 0.250 to 0.551 when contradicting documents arrive (epochs 4--13), (2) $V(t)$ drops to 0.351 when progressive resolution clears contradiction nodes (epochs 14--16), and (3) $V(t)$ rises again to 0.451 as subsequent documents introduce new contradictions (epochs 17--21). Individual runs confirm oscillatory dynamics: run-1 showed contradictions cycling between 0 and 3, with score recovering to 0.75 at resolution epochs then dropping to 0.45 as new contradictions appeared. This is consistent with the \emph{perpetual finality lifecycle} described in Section~\ref{sec:perpetual-finality}: the system does not converge once but continuously re-evaluates as evidence evolves.

Gate satisfaction across 94 evaluations: Gate~A (monotonicity) 70\%, Gate~B (evidence) 27\%, Gate~C (trajectory) 99\%, Gate~D (quiescence) 100\%, Gate~E (content) 100\%. The increased Gate~B satisfaction (from 12\% in the non-resolution variant to 27\%) reflects the periods where contradiction nodes were resolved.

\paragraph{Intra/cross-epoch analysis.} Each convergence point is now tagged with a \texttt{context\_seq} identifying the latest document injection. A single-run analysis (50 points, 7 evidence epochs) yielded: $\alpha_{\text{intra}} = -0.037$ (43 intra-epoch pairs), $\alpha_{\text{cross}} = +0.131$ (6 cross-epoch pairs). The positive $\alpha_{\text{cross}}$ reflects the V drop at resolution boundaries (context\_seq~63: contradictions~$\to$~0). The negative $\alpha_{\text{intra}}$ is a nuanced finding: contradictions are introduced \emph{within} an evidence epoch by the facts-worker extraction, not only at document boundaries. This means the theoretical regime boundary is governance-approved transitions specifically, not document injection events.

\paragraph{Per-dimension stalling.} Stalled-dimension detection identified all four dimensions as stalled in the final 5 evaluations: \texttt{goal\_completion} permanently at 0.00 (no goals resolved), \texttt{contradiction\_resolution} oscillating between 0.00 and 1.00, and \texttt{claim\_confidence}/\texttt{risk\_score\_inverse} saturated at 1.00. The scalar $V(t)$ collapses this into a binary oscillation between 0.250 and 0.550, masking the per-dimension dynamics. This provides concrete evidence for why a non-scalar convergence criterion (as advocated by SECP's Pareto filtering) would produce richer guarantees.
...
The trajectory demonstrates the spike-and-drop mechanism: score reaches 1.000 during epochs 1--9 (RESOLVED), then drops to 0.700 when contradicting documents arrive (epoch 10+). In the gates-enabled condition, Gate~A (monotonicity) immediately detects the non-monotonic drop and blocks re-entry to RESOLVED. Gate~B satisfaction drops from 100\% to 58\%. The system transitions to HITL/ESCALATED---never falsely remaining in RESOLVED after contradictions appear.
...
In YOLO mode, the system processed all documents through the pipeline with epoch advancing to 21 and 24 decision records produced. In MITL mode (with simulated human approvals), a similar progression occurred with automatic finality review approval.

In MASTER mode---the most restrictive governance level $\ell = \textsc{Master}$---all proposals were evaluated through the full reduction kernel (Definition~\ref{def:kernel}). Unlike the v1 implementation which short-circuited with an automatic \texttt{master\_override} approval, the v2 kernel applies the complete three-stage pipeline: policy check, lattice admissibility, and mode routing. Proposals that pass both policy and lattice checks receive \texttt{Accept(policy\_passed)}; proposals blocked by policy or lattice violations receive \texttt{Reject} rather than \texttt{Escalate}. This distinction is critical: MASTER does not bypass governance evaluation but rather routes through the strictest path, where any ambiguity (including lattice incomparability) results in rejection.
...
\midrule
YOLO   & 24 & 24 (100\%) & 0 & 0 & HITL \\
MITL   & 15 & 0 & 15 (100\%) & 0 & HITL \\
MASTER & 18 & 18 (100\%) & 0 & 0 & HITL \\
\bottomrule
\end{tabular}
\caption{Tier coverage by governance mode. YOLO routes all decisions through Tier~1 deterministic evaluation with yolo\_override where policy blocks. MITL escalates 100\% to Tier~2 (human-in-the-loop). MASTER rejects policy-blocked transitions deterministically at Tier~1. Combined coverage: Tier~1 = 73\%, Tier~2 = 26\%, demonstrating that multi-tier routing is exercised across configurations.}
\label{tab:exp7-tiers}
\end{table}

Across the three modes, the combined decision population exercised both Tier~1 and Tier~2 governance paths. MITL mode produced 100\% Tier~2 escalation (all proposals required human approval), while YOLO and MASTER operated at Tier~1. MASTER rejected 6/18 proposals (33\%) due to policy violations---specifically, high-drift transitions that YOLO overrode. This validates that the reduction kernel's mode-dependent routing produces observably different governance behavior: MASTER is the most restrictive (rejects on policy block), MITL requires human approval for every decision, and YOLO is the most permissive (overrides policy blocks with auditable justification). Assumption~4 is partially validated: Tier~1 and Tier~2 are exercised; Tier~3 (full LLM governance via the oversight agent choosing \texttt{escalateToLLM}) was not triggered because the oversight LLM consistently accepted the deterministic result.
...
\midrule
Baseline & 0.25 & 15 & 20 & 3 (15\%) & 0 & HITL \\
Inflate  & 0.55 & 14 & 34 & 16 (47\%) & 1 & HITL \\
Collude  & 0.55 & 32 & 44 & 7 (16\%) & 2 & HITL \\
\bottomrule
\end{tabular}
\caption{Adversarial defense results. \emph{Inflate}: single compromised facts agent inflates confidences, fakes resolutions. \emph{Collude}: facts + drift agents both compromised. YOLO ovr. = governance overrides where policy would have blocked. False RES = epochs of false RESOLVED state.}
\label{tab:exp8-results}
\end{table}

In \emph{inflate} mode, the honest drift agent detected the manipulation, producing 5.3$\times$ more governance overrides than baseline (16 vs 3). Gate~C (trajectory quality) caught post-inflation inconsistency at epoch~5. In \emph{collude} mode, both facts and drift agents were compromised: the system achieved false RESOLVED for 2 epochs (goal completion = 1.0, $V(t) = 0.0$) but self-corrected by epoch~5. The key defense mechanism is \emph{cycle-based re-extraction}: the facts-worker continuously re-extracts from source documents, overwriting adversarial mutations within 1--2 extraction cycles. This structural property---not designed as a Byzantine defense---limits adversarial impact to ephemeral windows. For sustained false finality, an adversary would need to compromise the facts-worker itself (3-agent collusion), exceeding the 2-agent collusion tested.

Gate~B (evidence coverage) provided additional defense: the all-gates-pass rate dropped from 71\% (baseline) to 49\% (collude), catching evidence inconsistencies that the adversary could not mask. Assumption~5 is partially validated: the cooperative model is structurally necessary (V(t) monotonicity was violated 9 times in collude mode, and brief false finality windows exist), but the system has unexpected defense-in-depth through cycle-based re-extraction.
...
CRDT commutativity & Partial & 6/24 permutations confluent; 18 diverge due to stale marking \\
...
\bottomrule
\end{tabular}
\caption{Local confluence sub-test results. Core CRDT operations (confidence ratchet, contradiction resolution) are fully commutative. Stale marking introduces order-dependence resolved by complete re-extraction. The governance kernel is a pure deterministic function.}
...
Stale marking breaks commutativity: when payload $A = \{X, Y\}$ is followed by payload $B = \{Y, Z\}$, node $X$ is staled; in the reverse order, $Z$ is staled. This is by design---stale marking implements last-writer-wins semantics to remove outdated claims. However, after a complete re-extraction (one cycle where all source documents are processed), all orderings converge to the same state. This eventual consistency is the same mechanism identified in Exp.~8 as an unintended Byzantine defense.
...
\paragraph{1. $V(t)$ tracks disagreement and responds to resolution (confirmed).} Across all conditions with contradictions ($n = 3$ for Exp.~1, $n = 3$ for Exp.~3), $V(t)$ increased reproducibly when contradicting documents entered the graph. In Exp.~1, $V$ rose from $0.250 \pm 0.001$ to $0.551 \pm 0.001$ upon contradiction arrival---a signal-to-noise ratio exceeding 100:1. Crucially, with progressive resolution, $V$ then \emph{decreased} from 0.551 to 0.351 when contradiction nodes were resolved (epochs 14--16), demonstrating that $V$ responds bidirectionally: increasing with contradictions and decreasing with resolutions. This confirms that the Lyapunov function tracks genuine epistemic state, not merely document count.
...
\paragraph{3. Governance mode affects coverage (confirmed).} Exp.~5 demonstrates the coverage--autonomy trade-off across governance modes. YOLO processes all documents (epoch = 21, 24 decisions). MITL occupies an intermediate position with human-escalated review. MASTER evaluates all proposals through the full reduction kernel---unlike the v1 implementation which bypassed evaluation, v2 MASTER applies the three-stage pipeline (policy, lattice, mode routing) and only accepts proposals that pass both policy and lattice admissibility checks. Policy violations and lattice incomparability produce \textsc{Reject}, not \textsc{Escalate}. This maps directly to the lattice ordering $\textsc{Master} \leq \textsc{Mitl} \leq \textsc{Yolo}$ in Definition~\ref{def:gov-lattice} and aligns with SECP's finding that stricter evaluation reduces coverage.
...
\paragraph{5. Multi-tier governance routing is exercised (confirmed).} Exp.~7 demonstrates that the three governance modes produce observably different tier coverage: YOLO routes 100\% of decisions through Tier~1 (with auditable overrides for policy blocks), MITL escalates 100\% to Tier~2 (human-in-the-loop), and MASTER rejects 33\% of proposals at Tier~1 due to policy violations. Combined across modes, Tier~1 and Tier~2 are both exercised. This confirms that the lattice ordering $\textsc{Master} \leq \textsc{Mitl} \leq \textsc{Yolo}$ produces structurally distinct governance behavior, not merely different labels on the same path.

\paragraph{6. Adversarial agents cause ephemeral---not permanent---false finality (partially confirmed).} Exp.~8 demonstrates that adversarial confidence inflation (single compromised agent) is caught by the honest drift agent (5.3$\times$ more governance overrides). Even with coordinated adversarial agents (facts + drift), false finality lasted only 2 epochs before cycle-based re-extraction flushed the adversarial mutations. The cooperative agent model (Assumption~5) is structurally necessary (brief false finality windows exist, $V(t)$ monotonicity is violated), but the system has unexpected defense-in-depth: source re-extraction acts as a practical Byzantine defense.

\paragraph{7. Partial confluence holds for CRDT operations (confirmed).} Exp.~9 demonstrates that the core semantic graph operations---confidence ratchet (max join), contradiction resolution (set-once), node upsert (content-matched)---are fully commutative. Stale marking is non-commutative (18/24 orderings diverge) but eventually consistent after complete re-extraction. The governance kernel is a pure deterministic function (80 evaluations, all identical). This validates the paper's claim of ``partial confluence'' and identifies stale marking as the specific operation preventing full commutativity.

\subsection{What Remains Unproven}

\paragraph{1. Partial convergence demonstrated; full $V \to 0$ not achieved.} With progressive resolution (contradiction nodes resolved at rounds 5--7), $V(t)$ dropped from 0.551 to 0.351 (a 36\% reduction) when resolutions took effect. However, subsequent documents re-introduced contradictions, causing $V$ to rise again to 0.451. The overall trajectory is oscillatory rather than monotonically decreasing: the system converges partially on each resolution, then diverges when new contradictory evidence arrives.

The intra/cross-epoch analysis reveals a subtlety: the regime boundary of Proposition~\ref{prop:monotonic} is finer than document injection. The $\alpha_{\text{intra}}$ was negative ($-0.037$) because contradictions are introduced \emph{within} an evidence epoch by the facts-worker extraction---a non-governance operation that occurs alongside governance-approved transitions. The proposition's precondition (``governance-approved transitions'') is formally correct, but the \texttt{context\_seq} proxy includes non-governance operations. A finer-grained partition---separating governance-approved transitions from facts-extraction side effects---would be needed to test the proposition directly.

Per-dimension analysis confirms a structural cause: the scalar $V(t)$ oscillates between two values (0.250 and 0.550) because only the \texttt{contradiction\_resolution} dimension changes; \texttt{goal\_completion} is permanently stalled at 0.00, and two other dimensions are saturated at 1.00. The scalar metric collapses this into apparent oscillation, when the underlying dynamics are: one dimension flipping, one stalled, two at target.

The v2 architecture partially addresses this limitation through the product lattice $\mathcal{M} = \mathcal{L} \times \mathcal{A}$ (Definition~\ref{def:product-lattice}), where the convergence rank $\mathcal{A} = (\mathbb{R}_{\geq 0})^k$ tracks each dimension independently. The reduction kernel's admissibility check (Definition~\ref{def:admissibility}) evaluates per-dimension regression rather than relying solely on the scalar aggregate, preventing transitions that improve the aggregate while regressing individual dimensions. However, the finality evaluator's ultimate decision still relies on the scalar goal score and gate predicates. A fully per-dimension finality criterion---requiring each dimension to independently satisfy its target---remains a direction for future work.
...
\paragraph{External validity.} All experiments use a single domain (M\&A due diligence). Generalization to other regulated domains (KYC/AML, clinical trials, regulatory compliance) is asserted by architectural argument (Section~\ref{sec:enterprise}) but not empirically validated.
...
\paragraph{Per-dimension finality (beyond lattice admissibility).} The v2 product lattice $\mathcal{M} = \mathcal{L} \times \mathcal{A}$ (Definition~\ref{def:product-lattice}) tracks each convergence dimension independently during governance evaluation, and the reduction kernel rejects transitions that regress any single dimension. However, the finality evaluator's ultimate decision still relies on a scalar goal score and gate predicates. Extending the finality gates to require per-dimension satisfaction would: (a) prevent declaring finality when one dimension is stalled (e.g., \texttt{goal\_completion} at 0.00) while others are saturated; (b) expose stalled dimensions as first-class diagnostic signals; (c) provide the per-dimension convergence rate $\alpha_d$ needed for meaningful ETA estimation; (d) align with SECP's non-scalar coordination, where Pareto filtering and constructive objections prevent the collapse of structured disagreement. This extension is straightforward within the lattice framework: finality requires $a_d \leq \epsilon_d$ for each dimension $d$ in the convergence rank, rather than a single scalar threshold.

\paragraph{Autonomous contradiction resolution.} The current agent ensemble extracts facts and detects contradictions but does not resolve them. Implementing a ``resolver agent'' that cross-references evidence and produces resolves edges would close the convergence loop and make Proposition~\ref{prop:monotonic} testable end-to-end within a single evidence epoch.
...
\begin{enumerate}
  \item \textbf{Architectural feasibility.} Declarative governance over shared immutable context is implementable with current technology (Postgres, NATS, LLMs, OpenFGA, native Rust via napi-rs) and produces a functioning coordination system with 121 Rust unit tests, 288 TypeScript tests, and 7 convergence benchmarks.
...
\begin{enumerate}
  ...
  \item \textbf{Local confluence (partially validated).} The connection to rewriting theory invokes Newman's Lemma, which requires local confluence as a precondition. Exp.~9 tested this property with six sub-tests across 24 permutations and 80 kernel evaluations. Result: core CRDT operations (confidence ratchet, contradiction resolution, node upsert) are fully commutative---the confidence ratchet implements max join (commutative, associative, idempotent), and contradiction resolution is set-once. Stale marking is non-commutative: 18/24 orderings diverge because the last extraction determines which nodes are marked irrelevant. However, all orderings converge to the same state after a complete re-extraction cycle (eventual consistency). The governance kernel is a pure deterministic function (same inputs produce identical outputs regardless of evaluation history). The paper's claim of ``partial confluence'' is validated: certified compatible transitions commute; full Church--Rosser confluence does not hold due to stale marking but is compensated by eventual consistency.

  \item \textbf{Monotonic progress precondition.} Proposition~\ref{prop:monotonic} and its corollary require that ``at least one convergence dimension makes strict progress'' per cycle. This precondition was never satisfied in the experiments: the \texttt{goal\_completion} dimension was permanently stalled at 0.00 because no goal-resolution agent exists. The proposition is therefore correct but its applicability depends on the agent ensemble being complete---a non-trivial requirement that the paper does not address.
...
This paper presents a paradigm for autonomous agent coordination based on declarative governance over shared immutable context. The core contributions are: (i) a formal convergence mechanism (Lyapunov disagreement function with five independent gates) that provides layered guarantees against premature finality, empirically validated with $n = 3$ repeated trials showing reproducible $V(t)$ trajectories and gate effectiveness against adversarial input; (ii) a bitemporal CRDT-inspired semantic graph with monotonicity invariants ensuring that shared state evolves toward resolution; (iii) a product lattice $\mathcal{M} = \mathcal{L} \times \mathcal{A}$ formalizing the governance-convergence coupling, with an Epoch Termination Theorem (Theorem~\ref{thm:termination}) guaranteeing finite convergence within evidence epochs via well-founded descent; (iv) a deterministic reduction kernel implementing the lattice ordering as a three-stage pipeline (policy check, lattice admissibility, mode routing) in compiled Rust, ensuring that all governance decisions are auditable, replayable, and independent of LLM stochasticity; (v) a perpetual finality lifecycle producing chains of Ed25519-signed certificates; (vi) a three-tier governance routing architecture ensuring continuous operation without LLM availability; and (vii) experimental validation on a realistic M\&A due-diligence scenario demonstrating both the capabilities and current limitations of the approach, validated by 121 Rust and 305 TypeScript tests.
...
\begin{itemize}
  ...
  \item Our v1 limitation on scalar convergence is partially addressed by the v2 product lattice $\mathcal{M} = \mathcal{L} \times \mathcal{A}$, which tracks convergence per-dimension during governance evaluation. The reduction kernel's admissibility check prevents transitions that regress individual dimensions, even when the scalar aggregate would improve. However, fully non-scalar \emph{finality} (requiring per-dimension satisfaction) remains future work, where SECP's Pareto filtering and constructive-objection requirements would provide the natural criterion.
...
The empirical claims are narrow and intentionally circumscribed (Sections~\ref{sec:boundaries} and \ref{sec:discussion}). Results from $n = 3$ repeated runs demonstrate reproducibility of $V(t)$ response to contradictions and gate mechanism effectiveness, but are not statistically powered distributional claims. The convergence guarantee (Proposition~\ref{prop:monotonic}) and the Epoch Termination Theorem (Theorem~\ref{thm:termination}) hold formally but were not demonstrated end-to-end: $V(t)$ never decreased to zero because no experiment produced full contradiction resolution. The product lattice and reduction kernel are validated by 121 Rust unit tests covering all governance paths (YOLO, MITL, MASTER) and admissibility classifications, but have not been stress-tested at production scale. The gate ablation confirms that gates prevent false finality on adversarial input. Five formal assumptions were experimentally validated: discretization and monotonic progress (Exp.~6), multi-tier governance routing (Exp.~7), cooperative agent model (Exp.~8), and local confluence (Exp.~9). All five were partially validated, identifying specific boundaries (stale marking for confluence, ephemeral false finality windows for adversarial resilience, Tier~3 non-triggering for governance routing). No production-scale validation ($N > 50$ documents) and no multi-domain validation have been performed.

\subsection{Final Assessment}

As LLM agents become increasingly capable of reasoning, planning, and hypothesis generation, the need for governance-based coordination---rather than pipeline-based orchestration---will grow. Regulated enterprises cannot adopt autonomous agents without the ability to audit, explain, and formally bound their behavior, not once but continuously as conditions evolve. The mechanisms that enable governance---shared state, declarative policy, formal convergence, and lattice-based admissibility---are precisely the mechanisms that make multi-agent coordination robust over indefinite operational horizons. The product lattice $\mathcal{M} = \mathcal{L} \times \mathcal{A}$ provides the formal foundation: governance strictness and convergence progress are jointly tracked in a well-founded ordering that guarantees finite stabilization within each evidence epoch, while the deterministic reduction kernel ensures that every governance decision is auditable, replayable, and independent of LLM stochasticity. Achieving full robustness requires combining this state-level convergence approach with the protocol-level coordination adaptability demonstrated by SECP. Neither alone is sufficient; together, they define the research agenda for governed autonomous agent systems in regulated environments.
