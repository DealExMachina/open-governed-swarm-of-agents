...
Per-dimension analysis confirms a structural cause: the scalar $V(t)$ oscillates between two values (0.250 and 0.550) because only the \texttt{contradiction\_resolution} dimension changes; \texttt{goal\_completion} is permanently stalled at 0.00 (goals are marked irrelevant by the stale marking mechanism during subsequent extraction cycles, as identified in Exp.~9), and two other dimensions are saturated at 1.00. The scalar metric collapses this into apparent oscillation, when the underlying dynamics are: one dimension flipping, one stalled by a structural issue, two at target.
...
\begin{enumerate}
  ...
  \item \textbf{Monotonic progress precondition.} Proposition~\ref{prop:monotonic} and its corollary require that ``at least one convergence dimension makes strict progress'' per cycle. This precondition was never satisfied in the experiments: the \texttt{goal\_completion} dimension was permanently stalled at 0.00 because goals are marked irrelevant by the stale marking mechanism (Exp.~9) during subsequent extraction cycles, preventing the resolution mechanism from advancing them. A resolver agent exists for contradictions but the goal dimension requires goal-aware stale protection. The proposition is correct but its applicability depends on resolving this structural interaction between stale marking and goal resolution.
