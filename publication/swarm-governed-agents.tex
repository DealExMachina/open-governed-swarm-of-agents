...
\paragraph{Empirical status of the corollary.} In early experiments, the precondition was not satisfied because the \texttt{goal\_completion} dimension was permanently stalled at 0.00: goals extracted by the facts worker were marked irrelevant by the stale marking mechanism during subsequent extraction cycles (Exp.~9), preventing goal resolution from advancing. This was a structural bug, not a theoretical limitation: goal and risk nodes are accumulative across documents and should not be subject to stale marking. The fix---protecting goal and risk nodes from stale marking in \texttt{syncFactsToSemanticGraph}---allows the resolution mechanism to advance \texttt{goal\_completion} when goals are resolved. With this fix, the corollary's precondition becomes satisfiable: within an evidence epoch where resolutions are provided, at least one dimension (contradiction resolution or goal completion) makes strict progress per cycle, and $V(t) \to 0$ is achievable.
...
Per-dimension analysis confirms a structural cause: the scalar $V(t)$ oscillates between two values (0.250 and 0.550) because only the \texttt{contradiction\_resolution} dimension changes while two other dimensions are saturated at 1.00. In earlier runs, \texttt{goal\_completion} was permanently stalled at 0.00 due to stale marking of goal nodes (Exp.~9); with goal-aware stale protection applied, this dimension can advance when goals are resolved, reducing the oscillation amplitude. The scalar metric collapses per-dimension dynamics into apparent oscillation, when the underlying behavior is one dimension flipping and the others at or near target.
...
\paragraph{Autonomous goal resolution.} The resolver agent cross-references evidence and produces resolves edges for contradictions, partially closing the convergence loop. However, the \texttt{goal\_completion} dimension remains stalled in experiments because goals extracted by the facts worker are marked irrelevant by subsequent extraction cycles (the stale marking mechanism identified in Exp.~9). Implementing goal-aware stale protection---or a goal-resolution agent that transitions goals to resolved status when supporting evidence reaches sufficient confidence---would make Proposition~\ref{prop:monotonic} testable end-to-end within a single evidence epoch.
...
\begin{enumerate}
  ...
  \item \textbf{Monotonic progress precondition.} Proposition~\ref{prop:monotonic} and its corollary require that ``at least one convergence dimension makes strict progress'' per cycle. In early experiments, this precondition was not satisfied because goals were staled by the extraction mechanism (Exp.~9). With goal-aware stale protection applied (goals and risks are now protected from stale marking as accumulative node types), the precondition becomes satisfiable when resolutions are provided. The proposition is correct; its practical applicability now depends on whether the resolver agent or experiment driver provides resolutions within each epoch.
