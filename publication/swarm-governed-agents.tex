...
\begin{itemize}
  ...
  \item Validation on a realistic M\&A due-diligence scenario with 121 Rust and 288 TypeScript tests, demonstrating convergence, contradiction resolution, and structured human review.
\end{itemize}
...
\mathrm{Adm}(m, m') = \begin{cases}
  \textsc{Admissible}           & \text{if } m' \leq_{\mathcal{M}} m \\
  ...
  \textsc{Incomparable}         & \text{if } \ell' \not\leq_{\mathcal{L}} \ell \;\wedge\; \mathbf{a}' \not\leq_{\mathcal{A}} \mathbf{a} \\
  \textsc{BothViolated}         & \text{otherwise (both components regress)}
\end{cases}
\]
Only \textsc{Admissible} transitions preserve descent in~$\mathcal{M}$.
\end{definition}
...
\begin{enumerate}
  ...
  \begin{itemize}
    ...
    \item $\textsc{ConvergenceViolation}$ or $\textsc{Incomparable}$: $\ell = \textsc{Master} \implies \textsc{Reject}$; otherwise $\textsc{Escalate}$.
  \end{itemize}
...
Each approved transition satisfies $m_{t+1} \leq_{\mathcal{M}} m_t$ by the admissibility check (Definition~\ref{def:admissibility}). Since $\mathcal{L}$ is finite and $\mathcal{A}$ is bounded in $[0,1]^k$ with granularity $\epsilon > 0$ (determined by the discrete evaluation intervals), the product $\mathcal{M}$ admits no infinite strictly descending chain. Strict descent follows from the requirement that at least one convergence dimension makes progress (Proposition~\ref{prop:monotonic}), and the governance level component $\ell$ is non-increasing. Thus the sequence terminates.
\end{proof}
...
The connection to rewriting theory \cite{baader1998} is direct: each governance-approved transition is a rewrite step, the well-founded ordering on $\mathcal{M}$ serves as the termination measure, and Newman's Lemma \cite{newman1942} provides the bridge from local confluence (adjacent governance decisions do not conflict) to global confluence (all governance evaluation orders produce the same final state). This framing also parallels Denning's lattice model \cite{denning1976}, where information flow is bounded by a lattice ordering---here, governance permission flow is bounded by $\mathcal{L}$, and convergence progress is bounded by $\mathcal{A}$.
...
\paragraph{Expected outcomes.} YOLO should produce maximum coverage but minimum autonomy (all objections are compensable by governance approval). MASTER ($\ell = \textsc{Master}$) should produce minimum coverage but maximum autonomy: proposals are evaluated through the full reduction kernel, and any policy violation or lattice incomparability produces a rejection rather than an escalation. MITL should occupy an intermediate position. This maps to SECP's coverage--autonomy spectrum: scalar aggregation ($\sim$YOLO) achieved coverage of 6/6; hard veto ($\sim$MASTER) restricts coverage to only fully admissible proposals; SECP non-scalar ($\sim$MITL) achieved 2--3/6.
...
\paragraph{2. Multi-path governance (not observed).} All decision records used \texttt{governance\_path = oversight\_acceptDeterministic}---the oversight LLM always agreed with the deterministic policy result. Even at medium drift levels (Exp.~4), the LLM did not choose to escalate to human review or override the deterministic result. This means the three-tier governance routing architecture is implemented and instrumented, but only Tier~1 was exercised. Triggering Tier~2 and Tier~3 paths would require proposals where the deterministic and LLM-based evaluations genuinely disagree, which may require adversarial prompt engineering or borderline drift levels not achieved by the current document corpus.
...
\begin{itemize}
  ...
  \item \textbf{MASTER} for highest-sensitivity scenarios (e.g., sanctions screening) where every proposal is evaluated through the full reduction kernel and any policy violation or lattice incomparability produces a deterministic rejection.
\end{itemize}
...
When business needs change, update the YAML governance rules. The change is auditable (the next decision record carries a new policy-version hash), testable (new rules can be evaluated against the existing event log in a dry-run mode), and live immediately. No code recompilation, no redeployment. The native Rust reduction kernel parses updated YAML at startup and evaluates all governance decisions deterministically---policy teams modify human-readable configuration files rather than writing code. Because the kernel is compiled as a Node.js addon via napi-rs, rule changes take effect on process restart without rebuilding the governance engine itself.
