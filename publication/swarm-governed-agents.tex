...
\title{Coordination of Governed Agents Over Shared Context:\\Declarative Governance and Lyapunov-Based Finality}

\author{
  Jean-Baptiste D\'ezard\thanks{Source code: \url{https://github.com/DealExMachina/swarm-of-governed-agents}}\\
  Deal ex Machina SAS\\
...
\begin{theorem}[Vector Finality Soundness]
\label{thm:vector-finality-soundness}
Under the CRDT monotonicity assumption (A3), if $F^*(t) = \text{true}$ then for all required
dimensions $d$:
\begin{enumerate}
  \item $e_d(t) \leq \epsilon_d$ (gap is within tolerance)
  \item $G_A^d(t) = \text{true}$ (monotonicity holds per dimension)
  \item $G_C^d(t) = \text{true}$ (trajectory quality holds per dimension)
  \item If $\text{VETO}_d = \text{true}$, then $F^*(t) = \text{false}$ whenever condition~1 fails
\end{enumerate}

Moreover, no state is reachable in which $S(t) \geq \theta_{\text{auto}}$ (scalar finality condition)
but $F^*(t) = \text{false}$ (vector finality fails), except when the system is operating in scalar-only
fallback mode (per-dimension finality disabled in configuration).
\end{theorem}

\begin{proof}[Proof sketch]
The vector finality predicate is a conjunction of independent per-dimension checks and global gates.
Each per-dimension check depends only on $\mu_d$ and the CRDT monotonicity property (A3), which guarantees
that $\mu_d$ is non-decreasing within an evidence epoch. Once a dimension crosses its threshold
$\tau_d - \epsilon_d$, it cannot regress (by A3), so the gap $e_d(t)$ cannot increase.

The converse---constructing a state where scalar passes but vector fails---is the core of the
non-compensability proof. A concrete witness is provided by the test case in
\texttt{vector\_finality\_blocks\_compensation} (see Section~\ref{sec:experimental-validation}):
with dimension scores $[1.0, 0.80, 1.0, 1.0]$, the scalar score is $0.30 \cdot 1.0 + 0.30 \cdot 0.80
+ 0.25 \cdot 1.0 + 0.15 \cdot 1.0 = 0.94 \geq 0.92$ (passes), but the contradiction resolution dimension
has gap $e_2(t) = 0.95 - 0.80 = 0.15 > \epsilon_2 = 0.01$ (fails veto check). This state is reachable
in the scalar-only system but blocked by vector finality.
\end{proof}
...
The dual-path implementation in the finality evaluator (Section~\ref{sec:implementation}) ensures that:
\begin{enumerate}
...
Three dedicated experiments validate the non-compensability guarantee (Proof Obligation PO-2):

\begin{enumerate}
  \item \textbf{exp-ab (Same-Session A/B Comparison):} Parallel evaluation of scalar vs. vector finality
  on M\&A and financial consolidation scenarios. Metrics: false finality rate, \texttt{compensation\_detected}
  count, divergence window magnitude. Demonstrates that vector finality blocks compensation cases
  (scalar passes, vector fails) while maintaining legitimate finality in uncompromised scenarios.

  \item \textbf{exp8-compensate (Compensation Attack Test):} Synthetic workload deliberately inflating
  one dimension (e.g., claim confidence to 1.0) while leaving another below threshold (e.g., contradiction
  resolution at 0.80). Validates that scalar finality $F(t)$ would declare \textnormal{\textsc{Resolved}}
  but vector finality $F^*(t)$ blocks it. Key metric: \texttt{compensation\_detected} flag raised
  in logs; finality certificate not issued.

  \item \textbf{Exp 9 sub-test 7 (Determinism and Replay):} Full re-extraction of evidence from
  a corpus (M\&A and financial scenarios), comparing dimension trajectories across runs. Confirms
  that per-dimension monotonicity (Gate $G_A^d$) holds across replay, validating assumption A3
  (CRDT monotonicity). Also validates that both $F(t)$ and $F^*(t)$ are deterministic when evaluated
  on the same graph state.
\end{enumerate}

The vector finality implementation comprises 20 Rust tests (Section~\ref{sec:implementation})
and 13 TypeScript tests, providing unit-level validation of gap computation, epsilon tolerance,
veto semantics, and per-dimension gate logic. The compensation-blocking case is exercised by
\texttt{vector\_finality\_blocks\_compensation}, which constructs the witness state $\mu = (1.0, 0.80, 1.0, 1.0)$
and verifies that $F(t) = \text{true}$ (scalar would pass) but $F^*(t) = \text{false}$ (vector blocks,
contradiction resolution veto fails).
...
\textbf{Scalar finality criterion (addressed).} The product lattice $\mathcal{M} = \mathcal{L} \times \mathcal{A}$ tracks convergence per-dimension during governance evaluation (the reduction kernel rejects transitions that regress any single dimension). The finality evaluator now supports both scalar and vector finality modes. When \texttt{per\_dimension\_finality.enabled} is true, the vector predicate $F^*(t)$ (Section~\ref{sec:formal-hardening}) requires each dimension to independently satisfy its threshold with per-dimension gates $G_A^{(d)}$ and $G_C^{(d)}$. This addresses the scalar aggregation that SECP~\cite{delachica2026} criticizes: both governance decisions and finality decisions are now fully lattice-aware. The scalar path is preserved as a configuration-driven fallback for backward compatibility.
...
\subsection{Formal Assumptions and Validation Status}

The following assumptions underpin the paper's formal claims. They are stated explicitly to prevent over-interpretation of the theoretical results. A sixth assumption---non-compensability (A6)---was previously open and is now closed by the vector finality implementation (Section~\ref{sec:formal-hardening}): the vector predicate $F^*(t)$ blocks finality when any required dimension fails its threshold, preventing scalar compensation artifacts.
...
\begin{enumerate}
  ...
  \item \textbf{Tier 2/3 governance routing (partially validated).} Exp.~7 demonstrated that the three governance modes produce distinct tier coverage: MITL mode escalated 100\% of proposals to Tier~2 (human-in-the-loop), while YOLO and MASTER operated at Tier~1. Combined across modes, Tiers~1 and~2 are both exercised, and MASTER's rejection behavior (30\% of proposals blocked by policy) confirms that mode-dependent routing is operational. Tier~3 (full LLM governance, where the oversight agent chooses \texttt{escalateToLLM}) was not triggered in any experiment: the oversight LLM consistently accepted the deterministic result. The system's behavior under genuine Tier~2/3 disagreement remains unknown.

  \item \textbf{Cooperative agent model (partially validated).} All formal guarantees assume cooperative agents (Threat Model, Section~\ref{sec:problem-setting}). Exp.~8 tested this assumption with two adversarial scenarios: single-agent inflation and coordinated collusion. Result: the monotonic confidence ratchet defends against downward manipulation but not upward inflation (confirmed). However, cycle-based re-extraction from source documents provides an unintended defense-in-depth: adversarial mutations are flushed within 1--2 extraction cycles. Single-agent inflation was caught by the honest drift agent (3$\times$ more governance overrides, 9 vs 3 baseline). Coordinated collusion produced 18 RESOLVED convergence snapshots across 31 epochs (vs 18 epochs baseline), with $V(t)$ oscillating between 0 and 0.25; each false RESOLVED window lasted only 1--2 extraction cycles before re-extraction flushed the adversarial state. Formal guarantees do not hold under adversarial conditions, but cycle-based re-extraction limits the practical impact of each manipulation. The cooperative assumption is structurally necessary for the formal theory, but the system's practical resilience exceeds what the theory guarantees. Formal Byzantine fault tolerance \cite{lamport1982,castro1999} remains unaddressed.
\end{enumerate}
...
\subsection{Formal Hardening: Per-Dimension Finality (Implemented)}
\label{sec:formal-hardening}

The formal hardening program aligns the finality semantics with the lattice semantics that governance already uses, and systematically validates the assumptions that underpin the paper's formal claims. This has been implemented as part of Issue~\#18.

\textbf{Per-dimension finality (implemented).} The scalar finality criterion has been replaced with a vector predicate that requires each dimension to independently satisfy its threshold:
\[
F^*(t) = \bigwedge_d \bigl[e_d(t) \leq \epsilon_d \;\wedge\; G_A^{(d)} \;\wedge\; G_C^{(d)}\bigr] \;\wedge\; G_B \;\wedge\; G_D \;\wedge\; G_E
\]
where $e_d(t) = \max(0,\, \tau_d - \mu_d(t))$ is the per-dimension gap, $\tau_d$ the per-dimension threshold, $G_A^{(d)}$ the per-dimension monotonicity gate, and $G_C^{(d)}$ the per-dimension trajectory quality gate. Global gates $G_B$ (evidence), $G_D$ (quiescence), $G_E$ (content) remain scalar. The implementation includes veto dimensions (contradiction resolution) whose failure unconditionally blocks finality, compensation detection (logging when scalar would pass but vector blocks), and a configuration-driven fallback to scalar finality for backward compatibility. This addresses the non-scalar convergence criterion that SECP~\cite{delachica2026} identifies as essential.

\textbf{Assumption matrix (documented).} Seven assumptions (A1--A7) underpin the formal claims: well-founded descent of $\mathcal{A}$ (A1, empirically validated), kernel determinism (A2, proven: 141 Rust tests), monotone CRDT evolution (A3, proven: SQL guards and CRDT invariants), progress precondition (A4, empirically validated: Exp.~6), local confluence boundary (A5, partially validated: Exp.~9), non-compensability (A6, \textbf{implemented}: vector finality predicate with 20 Rust + 13 TypeScript tests), and tier completeness (A7, partially validated: Exp.~7). Assumption~A6 was the critical open item and is now closed by the vector finality implementation.

\textbf{Proof obligations (PO-2 closed).} Four proof obligations were defined: (PO-1)~epoch termination with per-dimension rank function $r(t) = \sum_d \max(0, \tau_d - \mu_d(t))$; (PO-2)~per-dimension finality soundness; (PO-3)~eventual consistency safety; (PO-4)~routing completeness. PO-2 is proven by construction: the test \texttt{vector\_finality\_blocks\_compensation} provides a concrete witness where scalar score $= 0.94 \geq 0.92$ (passes) but contradiction resolution $= 0.80 < 0.95$ (vector blocks). The remaining obligations await extended experiments.

\textbf{Extended experiment program.} Five new experiments are planned: (E1)~scalar vs vector finality on paired corpora, measuring false finality rate and blocked compensation cases; (E2)~Tier-3 reachability via deterministic-vs-LLM disagreement corpus; (E3)~discretization sweep over $\epsilon$ sensitivity; (E4)~confluence boundary extension beyond Exp.~9; (E5)~adversarial compensation attack comparing scalar vs vector finality defenses. E5 is the key falsification experiment for PO-2.
